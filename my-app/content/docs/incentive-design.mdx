...
```mermaid
flowchart LR
    Admin["Protocol Admin\n(ProtocolAdminCap)"] -->|create_incentive_system| Caps["ProjectCap + PointCap"]
    Caps -->|create_scoreboard| Scoreboard["Shared Scoreboard"]
    Caps -->|share PointCap| Project["Project Contracts"]
    Project -->|add_point / subtract_point| Scoreboard
    Project -->|set/update linear time| Scoreboard
    Scoreboard -->|emit events| EventLog["Event Stream"]
    Scoreboard -->|read state| Users["Users / Integrators"]
```
